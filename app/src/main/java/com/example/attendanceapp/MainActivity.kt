        ...
        if (requestCode == ID_LOCATION_PERMISSION){
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED ||
                ...
                if (!isLocationEnabled()){
                    Toast.makeText(this, "Please Your turn on location", Toast.LENGTH_SHORT).show()
            ...
            if (isLocationEnabled()){
                LocationServices.getFusedLocationProviderClient(this).lastLocation.addOnSuccessListener { location ->
                    ...
                    val distance = calculateDistance(
                        ...
                        getAddresses()[0].latitude, getAddresses()[0].longitude
                    ) * 1000
...
//                    Log.d("address", "current location: $currentLat, $currentLong")
    ...
    private fun inputDataToFireBase(name: String) {
        ...
        attendanceRef.child(name).setValue(user)
            .addOnSuccessListener {
                tvCheckinSuccess.visibility = View.VISIBLE
                tvCheckinSuccess.text = "Check-in Sukses"
            }
...
//        val currentTime = java.util.Calendar.getInstance().time
    ...
    private fun getAddresses(): List<Address>{
        val destinationPlace = "stikes surabaya"
